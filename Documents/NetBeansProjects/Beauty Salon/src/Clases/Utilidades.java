...
public class Utilidades {
    ...
    public static String[][] filtrarEmpleados(int[] filtros, String[] values ,ArrayList<Usuarios> set){
        String[][] FR = null;
        ArrayList<Usuarios> rs = new ArrayList<>(set);
        
        
        if(filtros[0] == 1){
            var data = rs.stream()
                    .filter(p -> p.getNombre().equals(values[0]))
                    .map(p -> new Object[] {p.getId_usuario(), p.getNombre(), p.getTelefono(), p.getCorreo(), p.getRol(), p.getSueldo()})
                    .collect(Collectors.toList());
            
            rs.clear();
            for(int i = 0; i < data.size(); i++){
                Object[] datap = data.get(i);
                rs.add(new Usuarios(Integer.parseInt(datap[0].toString()), datap[1].toString(), datap[2].toString(), datap[3].toString(), Integer.parseInt(datap[5].toString()), datap[4].toString()));
            }
        }
        ...
        FR = new String[rs.size()][6];
        
        for(int i = 0; i < rs.size(); i++){
            FR[i][0] = String.valueOf(rs.get(i).getId_usuario());
            FR[i][1] = rs.get(i).getNombre();
            FR[i][2] = rs.get(i).getTelefono();
            FR[i][3] = rs.get(i).getCorreo();
            FR[i][4] = rs.get(i).getRol();
            FR[i][5] = String.valueOf(rs.get(i).getSueldo());
        }
